...
# ── Local-deploy path overrides (env vars win, then sensible Mac defaults) ──
_BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # local-deploy/
COMPANY    = os.environ.get('COMPANY_DIR',    os.path.join(_BASE, 'company'))
HARNESS    = os.environ.get('HARNESS_URL',    'http://localhost:44285')
MODES_YAML = os.environ.get('MODES_YAML',     os.path.expanduser('~/.bob/settings/custom_modes.yaml'))
...
def svc_status(name):
    # systemctl not available on macOS — check process name instead
    import platform
    if platform.system() != 'Linux':
        procs = subprocess.run(['pgrep', '-f', name], capture_output=True, text=True)
        return 'active' if procs.returncode == 0 else 'inactive'
    try:
...
def dashboard(filename):
    ...
    base_dir = os.environ.get('DASHBOARD_DIR', os.path.join(_BASE, 'my-dashboard'))
...
def list_dashboards():
    ...
    import glob
    _dash_dir = os.environ.get('DASHBOARD_DIR', os.path.join(_BASE, 'my-dashboard'))
    files = glob.glob(os.path.join(_dash_dir, '*.html'))
    return jsonify([{
